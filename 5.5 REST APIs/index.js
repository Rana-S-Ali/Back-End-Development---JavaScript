...
const yourBearerToken = "ea8b8264-7a01-4393-8067-c5717fabc2e3";
const config = {
...
app.post("/get-secret", async (req, res) => {
  ...
  if (!searchId) {
    return res.status(400).render("index.ejs", { errorMessage: "ID is required" });
  }
  try {
...
app.post("/put-secret", async (req, res) => {
  ...
  res.render("index.ejs", { content: JSON.stringify(result.data) });
    ...
    if (error.response && error.response.status === 404) {
      ...
      res.status(404).render("index.ejs",{content:"id was not-found"});
    } else {
      // Handle other errors
